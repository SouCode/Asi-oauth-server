...
class handler(BaseHTTPRequestHandler):
    ...
    def handle_discord_callback(self, code, state):
        ...
        try:
            ...
            discord_user_id = state.split('_')[0]
            
            # Redirect to Twitch OAuth
            twitch_oauth_url = self.generate_twitch_oauth_url(discord_user_id)
    ...
    def handle_twitch_callback(self, code, state):
        ...
        try:
            ...
            discord_user_id = state.replace('twitch_', '').split('_')[0]
            
            # Show success page
            self.send_success_page(discord_user_id, "Connected Successfully!")
            
        except Exception as e:
            self.send_error_page(f"Twitch callback error: {str(e)}")
    
    def generate_twitch_oauth_url(self, discord_user_id):
        """Generate Twitch OAuth URL"""
        state = f"twitch_{discord_user_id}_abc123"
        
        params = {
            'response_type': 'code',
            'client_id': os.getenv('TWITCH_CLIENT_ID', 'ccaxnoh0txxw0iulm4fxvq81km1dnd'),
            'redirect_uri': f"{os.getenv('VERCEL_URL', 'https://anasi-oauth-server.vercel.app')}/api/auth/twitch",
    ...
    def send_success_page(self, discord_user_id, message):
        """Send success page"""
        html = f"""
        <!DOCTYPE html>
        <html>
        <head>
            <title>Anasi - Connected!</title>
        ...
        <body>
            <div class="container">
                ...
                <div class="instructions">
                    ...
                    3. Use <code>/monitor &lt;streamer&gt;</code> to start!
                </div>
